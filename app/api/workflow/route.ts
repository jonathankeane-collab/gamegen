...
// In-memory store (Vercel KV in production)
...
- Apply the twist deeply: visuals, text, mechanics, sound effects via Web Audio API`,
      prompt: `Create a fully playable ${game} game${twist ? ` with this twist: "${twist}"` : ''}.
Infuse the twist into every aspect: visual style, colors, sounds, game objects, win/lose messages.`,
    })
...
`
      ...
      if (stdoutText) {
        ...
        if (!parsed.passed) {
          console.warn('Sandbox validation warnings:', parsed.checks)
        }
      }
    } finally {
    ...
    // Sandbox errors are non-fatal — continue with function step
    console.error('Sandbox error (non-fatal):', err)
  }

  // ── STEP 3: Function — finalize ──────────────────────────────────────
  runStore[runId] = { phase: 'function' }

  await new Promise(r => setTimeout(r, 300)) // simulate edge function processing
...
export async function POST(req: NextRequest) {
  ...
  // Run workflow async — respond immediately so caller can start polling
